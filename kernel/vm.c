...
walkaddr(pagetable_t pagetable, uint64 va)
{
  ...
  if((*pte & PTE_V) == 0){
	...
	//if(pa == 0){
		//printf("no pages free\n");
		//return 0;
	//}
	//mappages(pagetable,va,PGSIZE,pa,PTE_U|PTE_R|PTE_W);
	//return pa;
  }
  if((*pte & PTE_U) == 0)
    return 0;
...
uint64
uvmallocpte(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  uint64 a;
  pte_t *pte;
...
copyout(pagetable_t pagetable, uint64 dstva, char *src, uint64 len)
{
  ...
  while(len > 0){
	...
	if(va0 > myproc()->trapframe->sp && va0 < myproc()->sz && pa0 == 0){
		pa0 = (uint64)kalloc();
